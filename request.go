...
func (insta *Instagram) sendLoginRequest(o *reqOptions) (body []byte, h http.Header, err error, tokenHeader string) {
	if insta == nil {
		return nil, nil, fmt.Errorf("Error while calling %s: %s", o.Endpoint, ErrInstaNotDefined), ""
	}
	...
	tokenHeader = resp.Header.Get("Ig-Set-Authorization")
	return body, hCopy, err, tokenHeader
}
